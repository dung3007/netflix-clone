...
const SearchHistoryPage = () => {
  const [searchHistoty, setSearchHistory] = useState([])
  ...
  const handleDelete = async (entry) => {
    try {
      ...
      setSearchHistory(searchHistoty.filter((item) => item.id !== entry.id))
    } catch (error) {
      toast.error("Failed to delete search item")
    }
  }

  if (searchHistoty.length == 0) {
    return (
        ...
        <div className='grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4'>
          {searchHistoty.map((entry) => (
            <div key={entry.id} className='bg-gray-800 p-4 rounded flex items-start'>
              ...
              <Trash 
                className='size-5 min-w-4 ml-4 cursor-pointer hover:fill-red-600 hover:text-red-600'
